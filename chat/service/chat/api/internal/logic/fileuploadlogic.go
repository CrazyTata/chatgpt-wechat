...
func (f *FileUploadLogic) DealData(ctx context.Context, message milvus.Articles) (err error) {
	...
	film32 := make([]float32, milvus.VECTOR_DIM)
	...
	//数据库没有
	err = milvus.Save(f.svcCtx.Config.Embeddings.Milvus.Host, f.svcCtx.Config.Embeddings.Milvus.Username, f.svcCtx.Config.Embeddings.Milvus.Password, []milvus.Articles{message}, milvus.COLLECTION_NAME_ARTICLES)
	return
}
